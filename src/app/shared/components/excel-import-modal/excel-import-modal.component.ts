...
export interface ExcelImportModalConfig {
  ...
  customUploadFn?: (
    file: File,
  ) => Promise<ImportResult> | import("rxjs").Observable<ImportResult>;
...
export class ExcelImportModalComponent implements OnInit, OnDestroy {
    ...
    reader.onload = (e: any) => {
      this.ngZone.run(() => {
        try {
          ...
          this.parseError =
            "Không thể đọc nội dung file Excel: " + (err?.message || err);
          this.parsing = false;
        } finally {
          this.cdr.detectChanges();
        }
  ...
  confirmImport(): void {
    ...
    if (this.config.customUploadFn) {
      ...
      this.apiService
        .uploadFile<ImportResult>(this.config.importUrl, formData)
        .subscribe({
          next: (res: any) => {
            this.ngZone.run(() => this.handleImportSuccess(res));
          },
        ...
        if (this.progressPercent < 90) {
          ...
          if (this.progressPercent > 25 && this.progressPercent <= 65) {
            this.progressStatusMessage =
              "Máy chủ đang phân tích và lưu trữ dữ liệu...";
  ...
  private handleImportSuccess(res: ImportResult): void {
    ...
    if (res.errorCount === 0) {
      this.message.success(
        `Nhập thành công toàn bộ ${res.successCount} dòng dữ liệu!`,
      );
    } else {
    ...
    const errMsg =
      typeof err === "string"
        ...
        : err?.error?.message ||
          err?.error ||
          err?.message ||
          "Lỗi không xác định khi nhập file.";
    this.importResult = {
