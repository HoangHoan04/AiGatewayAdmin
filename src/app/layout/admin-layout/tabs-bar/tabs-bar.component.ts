...
export class TabsBarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  closeTab(id: string): void {
    ...
    setTimeout(() => {
      const currentTab = this.openTabs.find(
        (t) => t.id === this.ds.activeTabId,
      );
  ...
  closeOtherTabs(id: string): void {
    const keep = this.openTabs.filter(
      (t) => t.id === id || this.pinnedTabs.includes(t.id),
    );
  ...
  closeTabsToLeft(id: string): void {
    ...
    const keep = this.openTabs.filter(
      (_, idx) =>
        idx >= index || this.pinnedTabs.includes(this.openTabs[idx].id),
    );
    this.ds.setOpenTabs(keep);
